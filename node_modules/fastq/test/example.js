...
// Basic example
var queue = fastq(worker, 1);
queue.push('world', function(err, result) {
    if (err) throw err;
...
queue.unshift('world', function(err, result) {
    if (err) throw err;
    console.log('the result is', result);
});
...
var genericsQueue = fastq({
    base: 6
}, genericsWorker, 1);
genericsQueue.push(7, function(err, done) {
    if (err) throw err;
    console.log('the result is', done);
});
genericsQueue.unshift(7, function(err, done) {
    if (err) throw err;
    console.log('the result is', done);
});
...
var queue2 = queueAsPromised(asyncWorker, 1);
function asyncWorker(task) {
    return _asyncWorker.apply(this, arguments);
}
function _asyncWorker() {
    _asyncWorker = _async_to_generator(function(task) {
