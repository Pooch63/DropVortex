...
function start_game(options) {
    ...
    if (options.fixed_depth != undefined) {
        ...
        console.log(`You are playing against the bot. It has ${options.move_time}ms to think.`);
    }
    let Turn;
    (function (Turn) {
        Turn[Turn["BOT"] = 0] = "BOT";
    ...
    game_loop: while (true) {
        ...
        if (turn == Turn.BOT) {
            let time_taken = 0;
            let start = Date.now();
            let best = options.fixed_depth == undefined
                ? game.best_move(options.move_time, board_1.RED)
                : game.best_move_fixed_depth(depth, board_1.RED);
            ...
            while (true) {
                if (!(int < 1 || int > valid_moves.length || isNaN(int) || val === null)) {
                    break;
                }
                if (val == null)
                    process.exit();
    ...
    function process_command(val) {
        ...
        if (command_name == "set-board") {
            ...
            try {
                ...
                turn = game.board.get_player_turn() == board_1.RED ? Turn.BOT : Turn.HUMAN;
        ...
        if (command_name == "last") {
            boards.pop();
            if (boards.length > 0)
                game.board = boards[boards.length - 1];
            else
                game.board = new board_1.Board();
            return { processed: true, reprint: true };
        }
        if (command_name == "set-depth") {
            if (args[0] == null) {
                ...
                console.log(safe_1.default.red("You must specify a positive integer as a depth."));
            }
